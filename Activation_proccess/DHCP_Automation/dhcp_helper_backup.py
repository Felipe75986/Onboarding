...
def generate_dhcp_entries(mac_list, base_ip, start_host, rack_name, ru_position):
    ...
    for idx, mac in enumerate(mac_list, 1):
        ...
        hostname = f"ipmi_{rack_name}_ru{ru_position}_node{idx}"
...
def apply_dhcp_config_to_server(config_content, server_ip, username, password):
    ...
    try:
        ...
        backup_cmd = f"sudo cp /etc/dhcp/dhcpd.conf /etc/dhcp/dhcpd.conf.bak.{timestamp}"
        ...
        # Adiciona comentário e timestamp para identificação
        ...
        # 4. Criar arquivo temporário com nova configuração
        temp_file = f"/etc/dhcp/dhcpd.conf.{timestamp}"
        sftp = ssh.open_ftp()
