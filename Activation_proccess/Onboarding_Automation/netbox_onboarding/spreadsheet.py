...
from .csv_validators import validate_shape
...
def parse_onboarding_csv(
    ...
    # -- Shape gate --------------------------------------------------------
    shape_errors = validate_shape(df)
    if shape_errors:
        for err in shape_errors:
            logger.error("Spreadsheet shape error", reason=err)
        raise SpreadsheetError(
            f"Spreadsheet structure invalid ({len(shape_errors)} issue(s)): "
            + "; ".join(shape_errors)
        )
