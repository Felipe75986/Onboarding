...
def main() -> None:
    ...
    # -- Chassis mode (optional) -------------------------------------------
    chassis_name = os.getenv("RD_OPTION_CHASSIS_NAME")

    logger.info("Starting cable connections", file=file_path, chassis=chassis_name or "individual")
    client = NetboxClient(config, session, logger)

    # -- Parse & create ----------------------------------------------------
    delivery = parse_delivery_csv(file_path, logger)
    succeeded, failed = create_cables(client, delivery, logger, chassis_name=chassis_name)
